...
if (groupId) {
  $(`<div class="dialog-mygroup dialog-mygroup-title"><span>${lm.htmlEncode(lm.getCookie('groupName'))}</span>（<span class="fa fa-map-marker"></span>）<span class="fa fa-sign-out"></span></div>`).appendTo($('.dialog'));
  getGroupMembers({groupId: groupId}, function (result) {
    ...
    result.forEach(function (i, index) {
      ...
      $(`<div class="dialog-mygroup dialog-mygroup-member" data-userId="${i.userId}">
            ...
            <div>${userId == i.userId ? `<span style="color: #4BD0FD">自己${i.position == 1 ? '（团长）' : ''}</span>`
              : (i.position == 1 ? '<span style="color: red">团长</span>' : '饭友')}</div>
            <div>${i.createDate}</div>
        </div>`).appendTo($('.dialog')).css({top: top + 'px',left: left + 'px'})
        .animate({top: _top + 'px', opacity: 1}, result.length * 100 + 100 - index * 100).animate({top: _top - 5 + 'px'}, 'fast');
    });
    let dialogHeight = $('.dialog').height();
    if (result.length > 8) $('.dialog').height(dialogHeight > (180 + 50 * result.length) ? dialogHeight : (180 + 50 * result.length));
  });
} else {
  tip('试试加入一个饭团吧', true);
}
...
$('.searchGroup-btn').on('click', function () {
  ...
  if (!groupName) return;
  if (getStrlen(groupName) >= 15) return tip('请确保15个字符以内');
  if (groupName.length)
      ...
      if (result.length) {
        result.forEach(function (i, index) {
          ...
          $(`<div class="group-li" data-groupId="${i.groupId}">
              ...
              <div>${i.createDate}</div>
              <div class="join-group" onclick="joinGroup(${i.groupId})">加入</div></div>`).appendTo($('.dialog')).css({top: top + 'px',left: left + 'px'})
          .animate({top: _top + 'px', opacity: 1}, result.length * 100 + 100 - index * 100).animate({top: _top - 5 + 'px'}, 'fast');
        });
        var _top = top + height + (height + 10) * result.length + 35;
        $(`<div class="group-li back-to-mygroup">返回我的饭团</div>`).appendTo($('.dialog')).css({top: top + 'px',left: left + 'px'})
          .animate({top: _top + 'px', opacity: 1}, result.length * 100 + 100 - result.length * 100).animate({top: _top - 5 + 'px'}, 'fast').on('click', function () {
          location.reload();
        });
      } else {
...
$('.dialog-mygroup-title span').eq(1).on('click', function (){
  $('#map').show();
