...
if (groupId) {
  $(
    ...
    )}</span> <span class="fa fa-plus-circle" onclick="addFoodInput()"></span></div>`
  ).prependTo($('.dialog'));
  ...
  addFood({ groupId: groupId, foodName: foodName, location: location }, function (data) {
    if (data.code == 0) {
      ...
      $selfParent.animation(10, 5, function () {
        $selfParent.after(
          $(buildFoodCard(data.result.foodId, lm.getCookie('userName'), location, new Date(), foodName)).css({
            left: parseInt($selfParent.offset().left) + 'px',
            opacity: 1,
          })
        );
...
function buildFoodCard(foodId, userName, location, date, foodName) {
  return `<div class="food-li" data-foodid="${foodId}">
      ...
      <span onclick="addLocationInput(this)" data-location="${lm.htmlEncode(location)}">${lm.htmlEncode(
    location
  )}</span>
      <span>${lm.htmlEncode(lm.date.formatDate(date, 'yyyy-MM-dd'))}</span></div>
      <div class="food-li-name" onclick="addFoodNameInput(this)" data-foodname="${lm.htmlEncode(
        foodName
      )}"><span>${lm.htmlEncode(foodName)}</span></div>
  </div>`;
}
