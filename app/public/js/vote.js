...
if (groupId) {
  $(`<div class="dialog-mygroup-title"><span>${lm.htmlEncode(lm.getCookie('groupName'))}</span> <span class="fa fa-plus-circle" onclick="newVote(${groupId})"></span></span></div>`)
    .prependTo($('.dialog'));
      ...
      if (!msg.length) {  //开始投票
        ...
        getGroupMembers({groupId: groupId}, function (data) {
          ...
          result.forEach(function (i, index) {
            $(buildFoodCard(i.foodId, i.creator, i.location, new Date(i.createDate), i.foodName)).appendTo($('#foods').show()).css({left: '-500px'})
              .animate({
                left: _left + 10 + 'px',
                opacity: 1
              }, 500 / result.length * (index + 1)).animate({left: _left + 'px'}, 'fast');
          });
          $('#vote-tool').children('div:visible').eq(0).not('.reset-vote').css('marginLeft', _left + 'px');
        });
      } else {  //投票中
        ...
        if (action == 'against') {  //反对并重新投票
          ...
          if (!_index) {  //没有店铺可供投票，被全部否定
            tip('反对无效！兄嘚，没得吃啦');
            $(buildFoodCard(lastFoodInfo.foodId || lastFoodInfo.newFoodId, lastFoodInfo.creator, lastFoodInfo.location, new Date(lastFoodInfo.createDate), lastFoodInfo.foodName)).appendTo($('#foods').show()).css({left: '-500px'})
              .animate({
                left: _left + 10 + 'px',
                opacity: 1
              }, 500 / result.length).animate({left: _left + 'px'}, 'fast');
          } else {
          ...
          if (!_hasVote(msg, userId)) {
            ...
            $anonymity.show();
            var votedFood ={};
            msg.forEach((i) => {votedFood[i.foodId || i.newFoodId] = 1});
          ...
          if (voteNumber == lastFood.number) {  //投票结束
...
$('.dialog-mygroup-title span:first-child').on('click', function (){  //切换饭团
  lm.delCookie('groupId');
  lm.delCookie('groupName');
  location.reload();
});

$('.eat-num input').on('input propertychange', function () {  //投票人数设定
  $(this).siblings('label').find('span').text($(this).val());
});

$('#vote-now').find('span').eq(0).on('click', function () { //团长取消投票特权
  getGroupMembers({groupId: groupId}, function (data) {
    if (data) {
      var isManager = false;
      data.forEach((i) => {if (userId == i.userId && i.position == 1) isManager = true;});
      if (isManager && confirm('团长特权：\r确定关闭本次投票并重新开始吗')) return socket.emit('voteEnd', {groupId: groupId, force: true});
    }
  });
});
...
function showVoteHistory(msg) {
  ...
  newMsg.forEach((voteInfo, index) => {
    $(`<div class="vote-history-li">
          ...
          <span class="vote-history-li-against" style="color: ${voteInfo.foodId ? 'green' : 'red'}">${voteInfo.foodId ? '赞同了' : '重新提议了'}</span> 
          <span>${lm.htmlEncode(voteInfo.foodName)}</span><span>${lm.date.formatDate(new Date(voteInfo.createDate), 'hh:mm')}</span><div class="triangle-right"></div>
      </div>`).prependTo($('#vote-history').show()).fadeIn(100 * index);
  });
}
...
function vote(foodId) {
  socket.emit('vote', {foodId: foodId, groupId: groupId, anonymity: $('.btn-anonymity .fa-check').length, number: $('.eat-num input').val()});
}
