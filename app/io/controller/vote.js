  ...
  class Controller extends app.Controller {
    async vote() {
      ...
      if (!voteList) {
        ...
        voteList = [voteInfo];
        await app.redis.set(
          `${this.config._io.PREFIX_VOTE}:${params.groupId}`,
        ...
        this.ctx.socket
          .in(params.groupId)
          .emit('tip', `${params.anonymity ? '匿名用户' : this.ctx.session.username} 发起了投票`);
        ...
        if (lastFood.foodId == params.foodId || lastFood.newFoodId == params.foodId) {
          //赞成
          voteInfo.foodId = params.foodId;
          ...
          this.ctx.socket
            .in(params.groupId)
            .emit('tip', `${params.anonymity ? '匿名用户' : this.ctx.session.username} 赞同了 ${foodName}`);
          ...
          this.ctx.socket
            .in(params.groupId)
            .emit('tip', `${params.anonymity ? '匿名用户' : this.ctx.session.username} 重新提议了 ${foodName}`);
    ...
    async voteEnd() {
      ...
      let checkResult = await this.ctx.service.group.checkUserBelongsGroup(this.ctx.session.userId, params.groupId);
      if (!checkResult.length) return this.ctx.socket.emit('err', '你还不是该饭团的成员哦');
      if (params.force && checkResult[0].position != this.config.constants.position.MANAGER)
        return this.ctx.socket.emit('err', '暂无权限，请联系团长操作取消');
      let voteList = await app.redis.get(`${this.config._io.PREFIX_VOTE}:${params.groupId}`);
      if (voteList) {
        voteList = JSON.parse(voteList);
        let voteNumber = voteList.slice(_findAgainstIndex(voteList, this.ctx.session.userId)).length;
        if (voteNumber == voteList[voteList.length - 1].number || params.force) {
          //投票已结束或团长取消
